...
def main():
	...
	m = model.Model(comm, rank, size, params["longueur"], params["discretisation"], params["vent"], params["debut_feu"])
	...
	while(must_continue):
		...
		if rank == 0:
			if res:
				t_fin = time.time()
				temps_calcul = t_fin - t_deb
				print(f"Temps calcul : {temps_calcul}")
				if (m.time_step & 31) == 0:
					print(f"Time step {m.time_step}\n==============")
				t_deb = time.time()
				g.update(m.fire_map, m.vegetation_map)
				t_fin = time.time()
				temps_affichage = t_fin - t_deb
				print(f"Temps d'affichage : {temps_affichage}")
			else:
				must_continue = False
			...
			t_iter = time.time() - t_total_start  # ou bien, tu peux utiliser la différence d'itération
			resultats.append({
            "iteration": m.time_step,
            "temps_calcul": temps_calcul,
            "temps_affichage": temps_affichage,
            "temps_iteration": t_iter,
            "longueur": params["longueur"],
            "discretisation": params["discretisation"],
            "vent": params["vent"],
            "start": params["debut_feu"],
            "nombre_de_processus": size,
            "nombre_de_coeurs": size - 1
        })


		else:
			must_continue = None
		must_continue = comm.bcast(must_continue)
	
	if rank == 0:
		t_total_end = time.perf_counter()   # Fin de la simulation totale
		total_time = t_total_end - t_total_start
		print(f"\nTemps total de simulation : {total_time:.6f} secondes")
		print("Fin de la simulation", rank)
		...
		#except ImportError:
		#	print("Pandas non installé, impossible de créer le DataFrame.")
		#else:
		#	print("Nous aussi on a fini !", rank)
